...
class training:
    ...
    def train_minibatch(self, mainDQN, targetDQN, train_batch):
        ...
        #if ENVIRONMENT == 'v2':
        #state_array = np.vstack([np.append(np.array([x[0] for x in train_batch]),np.array([x[5] for x in train_batch]))])
        #next_state_array = np.vstack([np.append(np.array([x[3] for x in train_batch]),np.array([x[5] for x in train_batch]))])   #nstate 배열    [[BATCH_SIZE][INPUT_SIZE]]
